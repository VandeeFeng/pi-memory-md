...
- 可选的 `anchor.mode: "manual"` 守卫，阻止 `tape_handoff` 除非工具调用使用 `trigger: "keyword"` 或 `trigger: "manual"`
- 带有工具使用说明的 tape 提示
```
...
如果 `settings.tape.anchor.mode === "manual"`，主要 tape 提示会告诉 LLM 不要主动创建 `tape_handoff` 锚点，工具层也会拒绝 `tape_handoff` 除非调用使用 `trigger: "keyword"` 或 `trigger: "manual"`。关键词触发的 handoff 指令仍然会为匹配的提示覆盖该守卫，`/memory-anchor` 会为 `trigger: "manual"` 注入专门的隐藏指令。
...
- 如果该窗口内的助手工具调用总访问量少于 `MIN_SMART_ACCESS_SAMPLES`（硬编码为 `5`），则扩展到 `memoryScan[1]` 并重新扫描
- 最终排序：
